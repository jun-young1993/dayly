...
## ✅ TODO-T1 — 공유 화면에서 반복 이벤트 배지 표시 (P2, S) — DONE 2026-03-25
...
## ✅ TODO-T2 — 편집 화면에서 반복 타입 변경 지원 (P2, S) — DONE 2026-03-25
...
## TODO-BIZ-5-P3 — 위젯 페이지 인디케이터 애니메이션 효과 (P3, S)

**What:** Small/Medium/Large 위젯의 `< 1/2 >` 인디케이터에 페이지 전환 시 시각적 피드백 추가

**Why:** 현재 인디케이터는 정적(static)이라 페이지 전환 시 숫자만 바뀜. 사용자가 전환이 되었음을 인지하기 어려운 경우가 있음.

**Pros:** 전환 완료 피드백으로 UX 향상. SwiftUI `.animation()` 또는 불투명도 전환으로 구현 가능.
**Cons:** WidgetKit의 인터랙티브 애니메이션 지원이 제한적 — Timeline 기반이라 실시간 애니메이션 불가. 구현 방법 사전 검토 필요.

**Context:** `ios/DaylyWidget/DaylyWidget.swift`의 `DaylyPageIndicator` 구조체 수정.
WidgetKit에서는 `TimelineEntry` 전환 시 시스템이 크로스페이드 처리하므로 인디케이터 색상/크기 변화를 `.contentTransition`으로 힌트 제공하는 방식 검토.
Android 위젯은 RemoteViews 제약으로 별도 구현 필요.

**Effort:** S | **Priority:** P3
**Depends on:** BIZ-5 완료 (DaylyPageIndicator 추출) ✅
